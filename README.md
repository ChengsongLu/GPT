...
当前仓库已经完成 `Stage 0` 到 `Stage 7` 的基础能力：
...
- 应用启动后自动注册定时任务，执行 commit 同步、成员同步、日报生成与发送
- 前端日志页，可查看按天切分的系统日志
- `GET /health` 健康检查
...
应用自身也会把飞书发送历史落库，你可以直接在“飞书”Tab 下查看历史消息。

## 定时任务

当前服务启动后会自动注册以下任务：

- commit 同步：按页面配置的 `同步间隔（分钟）`
- 飞书成员同步：每天 `23:50`
- 日报生成并发送：每天 `00:00:00`

说明：

- 调度时区使用配置里的 `timezone`，当前默认 `Asia/Shanghai`
- 修改 GitLab 配置或飞书配置后，服务会自动重载任务
- 定时任务执行过程会写入 [logs/](/Users/lcs/Projects/GPT/logs)
